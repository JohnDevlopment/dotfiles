...
globalkeys = gears.table.join(
    ...
    awful.key(
       { modkey, "Control" }, "c",
       function()
	  awful.spawn.with_shell("emacsclient -c ~/.config/awesome/rc.lua")
       end,
       {description = "edit config in Emacs", group = "awesome"}
    ),
    ...
    awful.key({ modkey }, "b",
       function()
	  awful.spawn.with_shell("nice librewolf")
       end,
       {description = "launch Librewolf (launches on 2nd tag)", group = "applications"}),

    -- Browser (Brave)
    awful.key({ modkey, "Shift" }, "b",
       function()
	  awful.spawn.with_shell("nice -n 14 brave-browser")
       end,
       {description = "launch Brave browser (launches on 3rd tag)", group = "applications"})
)
...
-- Autostart
local function spawn_once(cmd, pidname)
   if not cmd then
      error("no command specified")
   elseif not pidname then
      -- Use cmd as pgrep argument
      pidname = cmd
   end

   -- Get list of pids for <pidname>
   local file = io.popen("pgrep " .. pidname)
   local pids = {}
   for line in file:lines() do
      table.insert(pids, line)
   end
   file:close()

   if #pids == 0 then
      -- Not spawned, so spawn process
      awful.spawn(cmd)
   end
end
...
-- Volume control
spawn_once("mate-volume-control-status-icon")
